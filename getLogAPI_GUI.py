...
logging.basicConfig(filename=os.path.join(os.getcwd(),"logfile.txt"), filemode='a', format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s', datefmt='%d-%m-%Y %H:%M:%S', level=logging.INFO)
...
def send_custom_request():
    if st.session_state.auth_method:
        try:
            ...
            #st.subheader(f"Response Status (Total Records: {len(rd['json'])})")
...
def get_logs():
    ...
    if st.session_state.state_type != None:
        try:
            ...
            if logs:
                try:
                    #new_logs = [log for log in logs if log[state_type] > state]
...
def send_logs(max_retries=3, delay=15):
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            for log in new_logs:
                ...
                for attempt in range(1, max_retries + 1):
                    try:
                        s.sendall(log_json.encode("utf-8"))
                        sent_count += 1
                        break  # Success, move to next log
                    except BrokenPipeError as e:
                        logging.warning(f"[SEND] Attempt {attempt} failed for log: {e}")
                        if attempt < max_retries:
                            time.sleep(delay)
                        else:
                            st.error(f"❌ Failed to send log after **{max_retries}** attempts: **{log}**")
                            logging.error(f"[SEND] Failed to send log after {max_retries} attempts: {log}")
        st.session_state.succ_send_logs = True

        st.success(f"✅ Sent **{sent_count}** logs to `{st.session_state.tcp_host}:{st.session_state.tcp_port}`")
        logging.info(f"[SEND] Sent {sent_count} logs to {st.session_state.tcp_host}:{st.session_state.tcp_port}")
        del st.session_state.new_logs

    except Exception as e:
        st.error(f"❌ Error sending logs: {e}")
        logging.error(f"[SEND] Error sending logs: {e}")
        st.session_state.succ_send_logs = False
...
if True:
    ...
    #auto_polling = st.checkbox("Enable auto polling", value=False)
    ...
    # Polling UI output placeholders
